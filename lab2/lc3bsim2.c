...
{
    int n = getBits(instruction, 11, 11);
    int z = getBits(instruction, 10, 10);
    int p = getBits(instruction, 9, 9);
...
void stb(int instruction)
{
    ...
    int br = getBits(instruction, 6, 8); //1c0
    int off1 = getBits(instruction, 0, 5); //3f //SIGN EXTEND
    int valInBr = Low16bits(CURRENT_LATCHES.REGS[br]); // value in br 
    int val = Low16bits(CURRENT_LATCHES.REGS[sr]) & 0x0FF; //sr[7:0]
    int location = Low16bits(off1 + valInBr);
    int bit = location & 0x0001;
    location = location >> 1;
...
void stw(int instruction)
{
    ...
    int br = getBits(instruction, 6, 8); //1c0
    int off1 = getBits(instruction, 0, 5); //3f
    int baserOff = Low16bits(CURRENT_LATCHES.REGS[br]); // value in br 
    ...
    off1 = Low16bits(off1 << 1);
    int location = Low16bits(off1 + baserOff);
    location = location >> 1;
    MEMORY[location][0] = val & 0x00FF; //sr[7:0]
    MEMORY[location][1] = ((val & 0xFF00) >> 8) & 0x00FF; //sr[15:8]  
}
...
{
    int num = (unsigned int) CURRENT_LATCHES.REGS[sr] >> bit;
...
void not_xor(int instruction){
    ...
    setnzp(solution);
    NEXT_LATCHES.REGS[dr] = Low16bits(solution);
}
...
void lea(int instruction)
{
    ...
    offset = Low16bits(offset << 1);
    int dr = getBits(instruction, 9, 11); //e00
    int location = Low16bits(CURRENT_LATCHES.PC + 0x02 + offset);
    NEXT_LATCHES.REGS[dr] = location;
...
void jmp_ret(int instruction){
    int baseR = getBits(instruction, 6, 8);
