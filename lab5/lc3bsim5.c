...
int GetLD_JSTORE(int *x)     { return(x[LD_JSTORE]); }
int GetGATE_VA(int *x)       { return(x[GATE_VA]); }
int GetGATE_JSTORE(int *x)   { return(x[GATE_JSTORE]); }
int GetVAMUX(int *x)         { return(x[VAMUX]); }
int GetTMUX(int *x)          { return(x[TMUX]); }
...
void rdump(FILE * dumpsim_file) {                               
    ...
    /*Delete this*/
    // printf("PSR_!5       : 0x%0.4x\n", CURRENT_LATCHES.PSR_15);
    // printf("PTBR         : 0x%0.4x\n", CURRENT_LATCHES.PTBR);
    // printf("VA           : 0x%0.4x\n", CURRENT_LATCHES.VA);
    // printf("JSTORE       : 0x%0.4x\n", CURRENT_LATCHES.JSTORE);
...
int translate_jstore()
{
    int next_state;
    switch (CURRENT_LATCHES.JSTORE & 0x3F)
    {
    case 2:
        next_state = 29;
        break;
...
int unknown_opcode_check(int next_state)
{
...
void eval_micro_sequencer() {
   ...
   */
    ...
    int next_state = GetJ(CURRENT_LATCHES.MICROINSTRUCTION);
    ...
    if (ird == 1) 
    {
        ...
        if (unknown_opcode_check(next_state) == TRUE)
        {
            vec = 0x05;
            NEXT_LATCHES.EXCV = 0x05;
            exception_enable = TRUE;
            next_state = INTERRUPT_EXCEPTION_START_STATE;
            // printf("IE Occurred, Vector: %x\n", vec);
        }
    }
    else if (GetGATE_JSTORE(CURRENT_LATCHES.MICROINSTRUCTION))
    {
        next_state = translate_jstore();
    }
    else
    {
        if (ie == TRUE)
...
void drive_bus() {
   ...
   */       
    ...
    {
        BUS = lshf(CURRENT_LATCHES.MODE, 3) + lshf(CURRENT_LATCHES.N, 2) + lshf(CURRENT_LATCHES.Z, 1) + CURRENT_LATCHES.P;
    }
    ...
    {
        if (GetVAMUX(CURRENT_LATCHES.MICROINSTRUCTION))
        {
            BUS = (CURRENT_LATCHES.MDR & 0x3E00) + (CURRENT_LATCHES.VA & 0x01FF);
        }
        else
        {
            BUS = (CURRENT_LATCHES.PTBR & 0xFF00) + rshfl(CURRENT_LATCHES.VA & 0xFE00, 8);
        }
    }
    else
    {
...
int protection_exception_check()
{
    if ((CURRENT_LATCHES.MDR & 0x08) == 0 && CURRENT_LATCHES.PSR_15 == 1)
    {
        return TRUE;
    }
    else
...
int page_fault_check()
{
    if ((CURRENT_LATCHES.MDR & 0x04) == 0)
    {
        return TRUE;
    }
    else 
...
void latch_mar()
{
    ...
    // {
        ...
        if (unaligned_exception_check(BUS) == TRUE)
        {   
            ie = TRUE;
            vec = 0x03;
...
void latch_mdr()
{   
    if (GetTMUX(CURRENT_LATCHES.MICROINSTRUCTION))
    {
        // Check for Protection Exception
        // printf("Entered exception checking\n");
        // printf("status of protection: %i\n", protection_exception_check());
        if (protection_exception_check() == TRUE && CURRENT_LATCHES.JSTORE != TRAP_MEMORY_ACCESS_STATE)
        {
            ie = TRUE;
            vec = 0x04;
        ...
        {
            NEXT_LATCHES.MDR = CURRENT_LATCHES.MDR | 0x01 | rshfl(CURRENT_LATCHES.JSTORE & 0x40, 5); // setting the reference and modified bits of PTE
        }
    }
    else
    {
...
void latch_jstore()
{
    NEXT_LATCHES.JSTORE = lshf(GetR_W(CURRENT_LATCHES.MICROINSTRUCTION), 6) + CURRENT_LATCHES.STATE_NUMBER;
}
...
void latch_mode()
{
    if (GetMODEMUX(CURRENT_LATCHES.MICROINSTRUCTION))
    {
        NEXT_LATCHES.MODE = 1;
    }
    else
...
void latch_datapath_values() 
{
   ...
   */       
    ...
    if (GetLD_JSTORE(CURRENT_LATCHES.MICROINSTRUCTION))
    {
        latch_jstore();
        latch_va();
    }
