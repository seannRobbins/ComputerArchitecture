...
void eval_micro_sequencer() {
   ...
   */
    ...
    else
    {
        ...
        case 3:
            next_state |= ((CURRENT_LATCHES.IR >> 11) & 0x01);
...
void gate_mdr() {
    ...
    */
    ...
    else
    {
        ...
        {
            mdr = (CURRENT_LATCHES.MDR & 0xFF00) >> 8;
        }
        // MAR[0] == 0
        else
        {
...
{   
    ...
    if (GetMIO_EN(CURRENT_LATCHES.MICROINSTRUCTION))
    {
        ...
        NEXT_LATCHES.MDR = (MEMORY[Low16bits(CURRENT_LATCHES.MAR) >> 1][1] << 8) + MEMORY[Low16bits(CURRENT_LATCHES.MAR) >> 1][0]; 
    }
    // Latching from BUS
    else
    {
